"""
BPLS-Application Format Generator
Validates, cleans, and formats BPLS-Application sheet data
"""
...
from config.schema import BPLS_APPLICATION_SCHEMA, CONDITIONAL_RULES, FieldType
...
class BPLSApplicationFormatGenerator:
    """Format generator for BPLS-Application sheet"""
    ...
    def process(self, filepath: str, auto_correct: bool = True) -> Dict:
        """Process BPLS-Application sheet"""
        print("\n" + "="*70)
        print("📝 BPLS-Application Format Generator")
        print("="*70)
        ...
        print("\n" + "="*70)
        print("✅ BPLS-Application Processing Complete!")
        print("="*70)
    ...
    def _read_data(self, filepath: str) -> List[Dict]:
        """Read BPLS-Application sheet - auto-detects header row"""
        try:
            from config.schema import BPLS_APPLICATION_SCHEMA
            expected_fields = list(BPLS_APPLICATION_SCHEMA.keys())
            
            df0 = pd.read_excel(filepath, sheet_name="BPLS-Application", header=0)
            cols0 = [str(c).strip() for c in df0.columns]
            has_fields_0 = sum(1 for f in expected_fields if f in cols0)
            
            df1 = pd.read_excel(filepath, sheet_name="BPLS-Application", header=1)
    ...
    def _clean_data(self, data: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        ...
        cleaner = DataCleaner()
        return cleaner.clean_sheet("BPLS-Application", data)
    ...
    def _validate(self, auto_correct: bool) -> List[ValidationResult]:
        ...
        for row_idx, row in enumerate(self.cleaned_data, start=2):
            for field_name, field_def in BPLS_APPLICATION_SCHEMA.items():
    ...
    def _validate_conditionals(self, row: Dict, row_idx: int) -> List[ValidationResult]:
        ...
        if "BPLS-Application" not in CONDITIONAL_RULES:
            return results

        for rule in CONDITIONAL_RULES["BPLS-Application"]:
    ...
    def _generate_csv(self) -> str:
        ...
        fieldnames = list(BPLS_APPLICATION_SCHEMA.keys())
        output_file = os.path.join(self.output_dir, "BPLS-Application_validated.csv")
    ...
    def _generate_reports(self):
        ...
        if errors:
            errors_file = os.path.join(self.output_dir, "BPLS-Application_errors.csv")
        ...
        if self.cleaning_log:
            cleaning_file = os.path.join(self.output_dir, "BPLS-Application_transformations.csv")
